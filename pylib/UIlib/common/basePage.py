...
class BasePage:
    """
    BasePage基础页面,使用OP模式，把元素定位写入yaml文件，以读取yaml文件的方式获取页面元素
    """

    def __init__(self, conf_path="configs/web_ele.yaml"):
        """
        初始化，获取driver,读取配置文件的数据
        :param conf_path: conf_path="configs/web_ele.yaml" 默认读取该文件
        """
        self.driver = Driver().get_driver()
        # 自定义的方法，初始化获取yaml文件中的页面元素，并赋值到当前页面类中作为属性
        self.__init_eles(conf_path)

    def __init_eles(self, conf_path):
        """
        动态读取配置文件的页面元素，把页面元素都赋值到当前页面类中作为属性
        :param conf_path:
        :return:
        """
        """
        方案一:使用当前页面继承的所有页面类作为元素配置项，去掉basepage和object
            意思就是：每个页面都继承祖宗页面的每个页面元素，缺点是每个页面的元素名称必须唯一，不能重名
        
         # 获取该类所有直属的祖宗，去掉basepage和object
    ...
    def find_element(self, selectors):
        ...
        """
        element = None
        # selector_by = selectors[0]  # 定位方式
        # selector_value = selectors[1]  # 定位值
        try:
            # 显示等待，判断元素是否存在
            WebDriverWait(
                # 传入浏览器对象
            ...
            logger.error("%s Timeout:查找元素超时" % get_dataTime())
            print(e)

        try:
            element = self.driver.find_element(*selectors)
            logger.info("%s Had find the element \' %s \' successfully " % (get_dataTime(), element.text))
    ...
    def find_elements(self, selectors):
        ...
        elements = None
        try:
            elements = self.driver.find_elements(*selectors)
            logger.info("%s Had find the elements \' %s \' successfully " % (get_dataTime(), elements.text))
        except NoSuchElementException as e:
            logger.error("%s NoSuchElementException:%s" % (get_dataTime(), e))
            self.get_windows_img()
        return elements
    ...
    def get_windows_img(self):
        ...
        #
        # # 获取当前文件所在目录的上一级目录
        # cur_dir1 = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))
        # print(cur_dir1)
        '''
        ...
        if not isExists:
            try:
                ...
                logger.error("%s Failed new build folder %s" % (get_dataTime(), e))
        # # 获取时间作为字符串，用作保存图片的名称
    ...
    # 通过input标签上传文件
    def uploadFile_to_input(self, selector, path):
        """
        通过input元素标签上传文件
        :param selector: 元素
        :param path: 路径默认值
